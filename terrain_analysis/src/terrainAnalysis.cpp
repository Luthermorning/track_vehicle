// 这部分功能主要的思路是将点云转换到车辆坐标系下，根据需要建立地面体素网格
// 然后将点云信息填充网格，根据车辆的特性筛选出合适的点云，求解最小的点云的地面高度，基于此，将小于车辆高度，大于附近的最小值
// 将这部分点云信息放到地面的点云信息中，从而将地面的点云提取出来发布出去。
#include <math.h>
#include <ros/ros.h>
#include <stdio.h>
#include <stdlib.h>
...
// terrain voxel parameters 地面体素参数
...
// planar voxel parameters 平面体素参数
...
int noDataInited = 0; // 车辆初始位置的标志位 0-未赋值，将收到的第一个车辆位置赋值；
...
int main(int argc, char** argv)
{
    ...
    while (status) {
        ...
        if (newlaserCloud) {
            ...
            // 激光雷达数据栈
            // 下面这个for主要是将激光雷达的点云数据填充到terrainVoxelCloud,转换为平面体素网格．
            ...
            // 主要是对激光雷达点云信息进行过滤，从而将有用的点云信息存储到terrainVoxelCloudPtr
            ...
            // 下面这个if else主要是对点云信息的ｚ轴高度进行筛选，将最小的ｚ轴高度存储到tplanarVoxelElev
            if (useSorting) {
                for (int i = 0; i < planarVoxelNum; i++) {
            ...
            // 下面这个for将点云信息进行滤波与高度处理，将处理结果存储到terrainCloudElev中，然后将地面的点云信息发布出去
